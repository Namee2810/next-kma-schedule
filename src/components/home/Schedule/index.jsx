...
export default function Schedule({ schedule, date, setDate }) {
  ...
  const handleClickPrevMonth = () => {
    let prevMonth = date.get("month") - 1,
      year = date.get("year")
    setDate(
      moment({
  ...
  const handleClickNextMonth = () => {
    let nextMonth = date.get("month") + 1,
      year = date.get("year")
    setDate(
      moment({
  ...
  const getSubjects = (value) => {
    return schedule.filter(
      (item) =>
        item.day?.[0] === value.get("date") &&
        item.day?.[1] === value.get("month") + 1 &&
        item.day?.[2] === value.get("year")
    )
  }
  ...
  const headerRender = () => {
    return (
      ...
      >
        <div className="cursor-pointer mt-5" onClick={handleClickPrevMonth}>
          <ChevronLeftIcon />
        </div>
        <div
          className="font-700 mx-24 text-20 text-primary cursor-pointer"
          onClick={handleClickDate}
          id="calendar-date"
        >
          {date.format("MMM, YYYY")}
        </div>
        <div className="cursor-pointer mt-5" onClick={handleClickNextMonth}>
          <ChevronRightIcon />
        </div>
      </div>
    )
        ...
        subjects.map((item, idx) => (
          ...
          >
            <Popover
              placement="right"
              trigger="hover"
              title={<b>{item.className}</b>}
              content={
                <>
                  <div>
                    <b className="text-underline text-primary">Thời gian:</b>
                    <span className="font-500 ml-8">
                      {item.day.join("/")}{" "}
                      {getTimeFromLesson([item.lesson[0], item.lesson[2]])}
                    </span>
                  </div>
                  <div className="mt-5">
            ...
            >
              <b className="text-primary text-underline">
                {getTimeFromLesson([item.lesson[0]])}
              </b>
    ...
    <>
      ...
      {!fullScreen &&
        (subjectsToday.length ? (
          <div className="my-16 d-flex flex-col align-center">
              ...
              >
                ...
                <div className={styles.calendar_item_detail}>
                  <div>
                    ...
                    <div className={styles.calendar_item_value}>
                      {item.className}
                    </div>
                  </div>
                  <div>
          ...
          <div className="my-16">
            <span className="background-red text-white border-r20 p-8 fade">
