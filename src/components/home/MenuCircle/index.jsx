...
function getTransformStyle(idx, max) {
  ...
  let x = -Math.round((Math.cos(angle) * length)) + "px", y = -Math.round(Math.sin(angle) * length) + "px";
...
export default function MenuCircle({ schedule, setDate }) {
  ...
  return (
    <>
      <div className={[styles.menu, "cursor-pointer"].join(" ")}
        ref={ref}
      ...
      <div className={styles.menu_items}>
        {items.map((item, idx) =>
          <Tooltip
            key={idx} trigger="hover"
            title={item.title} placement="left"
          >
