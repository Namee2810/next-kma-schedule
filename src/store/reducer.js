...
export const signIn = createAsyncThunk("user/signIn", async ({ username, password }, { rejectWithValue }) => {
  try {
    const res = await (await axios.post("https://heroku-kma-schedule-api.herokuapp.com/login", { username, password })).data;
    return res;
  }
  catch (err) {
...
export const signInWithToken = createAsyncThunk("user/signInWithToken", async (token, { rejectWithValue }) => {
  try {
    ...
    [signIn.fulfilled]: (state, { payload }) => {
      ...
      switch (payload.status) {
        ...
        case 200: {
          ...
          const token = jwt.sign({ studentProfile, schedule }, process.env.NEXT_PUBLIC_SECRET_KEY);
